Shader "Custom/Campo" 
{
	...
	Properties 
	{
		...
		_MainTex2 ("Textura Base2 (RGB)", 2D) = "white" {}
		
		_MainTex3 ("Textura Base3 (RGB)", 2D) = "white" {}
	...
	SubShader 
	{
		...
		void surf (Input IN, inout SurfaceOutput result) 
		{
			...
			if ( PixelEscalaCinza >= 0.7)
			{
				half4 PixelOriginal = tex2D (_MainTex, IN.uv_MainTex);
				result.Albedo = PixelOriginal.rgb;
				result.Alpha = PixelOriginal.a;
			}
			else if(PixelEscalaCinza >= 0.3 )
			{
				half4 PixelOriginal = tex2D (_MainTex2, IN.uv_MainTex2);
				result.Albedo = PixelOriginal.rgb;
				result.Alpha = PixelOriginal.a;
			}
			else
			{
				half4 PixelOriginal = tex2D (_MainTex3, IN.uv_MainTex3);
				result.Albedo = PixelOriginal.rgb;
				result.Alpha = PixelOriginal.a;
			}
